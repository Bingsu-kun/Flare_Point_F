  ...
  methods: {
    getLikeCount: async function(markerId) {
      try {
        await axios({
          method: 'POST',
          url: 'https://3.34.252.182:8080/marker/thiscount',
    ...
    getMadeBy: async function() {
      ...
      try {
        await axios({
          method: 'GET',
          url: `https://3.34.252.182:8080/fisher/${id}`,
          withCredentials: true
        }).then((res) => {
          if (res.data.success === false) {
    ...
    like: async function() {
      ...
      if (!sessionStorage.getItem('id'))
        ...
        try {
          await axios({
            method: 'POST',
            url: 'https://3.34.252.182:8080/marker/like', // 좋아요
    ...
    dislike: async function() {
      ...
      try {
        await axios({
          method: 'DELETE',
          url: 'https://3.34.252.182:8080/marker/dislike', //좋아요 취소
          headers: { Authorization: `Bearer ${sessionStorage.getItem('apiToken')}` },
          data: { markerId: id },
          withCredentials: true
        }).then((res) => {
    ...
    deleteMarker: async function() {
      ...
      try {
        await axios({
          method: 'DELETE',
          url: 'https://3.34.252.182:8080/marker/delete',
          headers: { Authorization: `Bearer ${sessionStorage.getItem('apiToken')}`},
